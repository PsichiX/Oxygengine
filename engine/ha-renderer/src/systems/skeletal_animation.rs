use crate::{asset_protocols::skeletal_animation::*, components::skeletal_animation_instance::*};
use core::{
    app::AppLifeCycle,
...
pub struct HaSkeletalAnimationSystemCache {
    pub(crate) map: HashMap<String, (SkeletalAnimationAsset, AssetId)>,
    table: HashMap<AssetId, String>,
}

pub type HaSkeletalAnimationSystemResources<'a> = (
    WorldRef,
    &'a AppLifeCycle,
    &'a AssetsDatabase,
    &'a mut HaSkeletalAnimationSystemCache,
    Comp<&'a mut HaSkeletalAnimationInstance>,
);
        ...
        $animation
            ...
            .find_map(|rule| {
                match rule {
                    ...
                    SkeletalAnimationRule::BlendSpace {
                        conditions,
                        axis_scalars,
                        blend_states,
                        change_time,
            ...
            if &$active.state != name {
                ...
                if let Some(state) = $animation.states.get(name) {
                    match &state.sequences {
                        SkeletalAnimationStateSequences::Single(name) => {
                        ...
                        SkeletalAnimationStateSequences::BlendSpace { sequences, .. } => {
...
macro_rules! update_blend_weights {
    ($state: expr, $active: expr, $skeleton: expr) => {
        match &$state.sequences {
            SkeletalAnimationStateSequences::Single(sequence) => {
                if let Some(data) = $active.current_sequences.get_mut(sequence) {
                    data.blend_weight = 1.0;
                }
            }
            SkeletalAnimationStateSequences::BlendSpace {
                axis_scalars,
                sequences,
            } => {
...
pub fn ha_skeletal_animation(universe: &mut Universe) {
    let (world, lifecycle, assets, mut cache, ..) =
        universe.query_resources::<HaSkeletalAnimationSystemResources>();

    for id in assets.lately_loaded_protocol("skelanim") {
        if let Some(asset) = assets.asset_by_id(*id) {
            let path = asset.path();
            if let Some(asset) = asset.get::<SkeletalAnimationAsset>() {
                cache.map.insert(path.to_owned(), (asset.to_owned(), *id));
                cache.table.insert(*id, path.to_owned());
            }
        }
    }
    for id in assets.lately_unloaded_protocol("skelanim") {
    ...
    for (_, skeleton) in world.query::<&mut HaSkeletalAnimationInstance>().iter() {
        skeleton.signals.clear();

        if let Some((animation, _)) = cache.map.get(&skeleton.animation) {
            if skeleton.playing && skeleton.active.is_none() {
                if let Some(name) = &animation.default_state {
                    skeleton.play(name);
                }
            }

            if let (true, Some(active)) = (skeleton.playing, skeleton.active.as_mut()) {
                if let Some(state) = animation.states.get(&active.state) {
                    let selected = try_select_state!(animation, skeleton, state);
                ...
                if let Some(state) = animation.states.get(&active.state) {
                    ...
                    skeleton.playing = playing;
                    update_blend_weights!(state, active, skeleton);
                }
