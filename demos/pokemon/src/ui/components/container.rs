...
widget_component! {
    pub container(key, props, listed_slots) {
        ...
        let background_material = ImageBoxImage {
            ...
            source_rect: Some(Rect {
                left: if container_props.fixed_left { 384.0 } else { 0.0 },
                right: if container_props.fixed_right { 768.0 - 384.0 } else { 768.0 },
                top: 0.0,
                bottom: 512.0,
            }),
            scaling: ImageBoxImageScaling::Frame(ImageBoxFrame {
                source: Rect {
                    left: if container_props.fixed_left { 0.0 } else { 384.0 },
                    right: if container_props.fixed_right { 0.0 } else { 384.0 },
                    top: 0.0,
                    bottom: 0.0,
                },
